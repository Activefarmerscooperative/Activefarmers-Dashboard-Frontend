...
import { BankDetails, BankList, GuarantorDetails, UpdateBankDetails } from '../../../utils/api/member';
...
const AccountGuarantor = () => {
  ...
  useEffect(() => {
    ...
    setGuarantorDetails(data[1].guarantor_details)
          ...
          <form action="" className="d-flex flex-column align-items-center">
            ...
            <div className="d-flex">
              <div className="form-group d-flex flex-column mx-3">
                ...
                <input type="text" name="guarantorname" disabled={!editGuarantor} value={guarantorDetails.guarantor?.full_name} placeholder="Joseph Ojih" />
              </div>
              <div className="form-group d-flex flex-column mx-3">
                <label htmlFor="">Guarantor's Phone Number</label>
                <input type="tel" name="gphone" value={guarantorDetails.guarantor?.phone} disabled={!editGuarantor} placeholder="+2348123456789" />
              </div>
              <div className="form-group d-flex flex-column mx-3">
                <label htmlFor="">Guarantor's Email Address</label>
                <input type="email" name="email" value={guarantorDetails.guarantor?.email} disabled={!editGuarantor} placeholder="joe@yahoo.com" />
              </div>
            </div>
            <div className="d-flex mt-4">
              <div className="form-group d-flex flex-column mx-3">
                <label htmlFor="">Guarantor's Residential Address</label>
                <input type="text" name="gaddress" value={guarantorDetails.guarantor?.address} disabled={!editGuarantor} placeholder="Airforce Base, Jimeta-Yola Adamawa" />
              </div>
              <div className="form-group d-flex flex-column mx-3">
                <label htmlFor="">Guarantor's Occupation</label>
                <input type="text" name="company" value={guarantorDetails.guarantor?.occupation} disabled={!editGuarantor} placeholder="FullStack Developer" />
              </div>
              <div className="form-group d-flex flex-column mx-3">
                <label htmlFor="">Gender</label>
                <select name="" value={guarantorDetails.guarantor?.gender} disabled={!editGuarantor}>
